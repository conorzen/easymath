...
def basic_examples():
    ...
    for expr, vars_dict in examples:
        ...
        vars_str = ", ".join(f"{k}={v}" for k, v in vars_dict.items()) if vars_dict else "no variables"
...
def finance_examples():
    ...
    calc.define("simple_interest", "P * (1 + r*t)", 
                "Simple interest: P=principal, r=annual rate, t=years")
    calc.define("monthly_payment", "P * (r*(1+r)^n) / ((1+r)^n - 1)", 
                "Monthly loan payment: P=principal, r=monthly rate, n=total payments")
    calc.define("future_value_annuity", "PMT * (((1+r)^n - 1) / r)", 
    ...
    payment = calc.calculate("monthly_payment", P=principal, r=monthly_rate, n=total_payments)
...
def geometry_examples():
    ...
    geometry_formulas = [
        ...
        ("triangle_area_heron", "sqrt(s*(s-a)*(s-b)*(s-c))", "Heron's formula (s=semi-perimeter)"),
...
def statistics_examples():
    ...
    calc.define("variance_population", "sum_squared_deviations / n", "Population variance")
    ...
    calc.define("coefficient_variation", "(σ / μ) * 100", "Coefficient of variation (%)")
    ...
    variance_pop = calc.calculate("variance_population", sum_squared_deviations=sum_squared_deviations, n=n)
    variance_samp = calc.calculate("variance_sample", sum_squared_deviations=sum_squared_deviations, n=n)
...
def advanced_examples():
    ...
    # Complex formulas
    advanced_formulas = [
        ("normal_distribution", "(1/sqrt(2*π*σ^2)) * e^(-0.5*((x-μ)/σ)^2)", "Normal distribution PDF"),
        ("compound_annual_growth", "((FV/PV)^(1/n)) - 1", "Compound Annual Growth Rate"),
        ("black_scholes_call", "S*N_d1 - K*e^(-r*T)*N_d2", "Black-Scholes call option (simplified)"),
        ("loan_remaining", "P * ((1+r)^n - (1+r)^p) / ((1+r)^n - 1)", "Remaining loan balance"),
