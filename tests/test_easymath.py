...
from easymath.exceptions import (
    EasyMathError, ExpressionError, VariableError, 
    FunctionError, MathematicalError
)
...
class TestComplexExpressions:
    ...
    def test_mixed_operations(self):
        ...
        result = self.calc.eval("2PI * SQRT(x^2 + y^2) + SIN(α)", 
                               x=3, y=4, α=math.pi/6)
...
class TestUtilityFunctions:
    ...
    def test_is_valid_expression(self):
        ...
        assert is_valid_expression("2x + 1") == True
        assert is_valid_expression("SIN(PI/2)") == True
        assert is_valid_expression("2x +") == False
        assert is_valid_expression("((x + 1)") == False
